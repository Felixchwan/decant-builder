...
describe("Aurelian analytics foundation", () => {
  ...
  it("rejects prohibited keys nested inside an object payload field", () => {
    const received = [];
    const analytics = createAnalytics({
      provider: {
        track(eventName, payload) {
    ...
    expect(
      analytics.track(ANALYTICS_EVENTS.COMPOSER_GENERATION_FAILED, {
        errorCategory: { code: "x", stack: "raw trace" },
        durationMs: 10,
        source: "system",
      })
    ).toBe(false);
  ...
  it("rejects prohibited keys nested inside an array-of-objects payload field", () => {
    const received = [];
    const analytics = createAnalytics({
      provider: {
        track(eventName, payload) {
    ...
    expect(
      analytics.track(ANALYTICS_EVENTS.PROPOSAL_APPLIED, {
        proposalPerfumeIds: [{ id: 1, notes: "private" }],
        replacedPerfumeIds: [],
        slotCountAfter: 6,
        totalPointsAfter: 12,
        source: "system",
  ...
  it("builds merchant context without contact data", () => {
    expect(
      buildAnalyticsContext({
        ...
        finalization: { whatsappNumber: "5281XXXXXXXX" },
      })
    ).toEqual({
      merchantId: "aurelian",
      locale: "es-MX",
  ...
  it("a valid provider call remains non-blocking (synchronous, no promise required)", () => {
    ...
    const result = analytics.track(ANALYTICS_EVENTS.APP_LOADED, { source: "system" });
