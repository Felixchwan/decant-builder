...
const appSource = readFileSync(new URL("../BuilderRuntime.jsx", import.meta.url), "utf8");
const hostCss = readFileSync(new URL("../../../../src/App.css", import.meta.url), "utf8");
...
describe("Builder stylesheet ownership", () => {
  it("loads host and Builder styles once in the approved production order", () => {
    expect(mainSource).toMatch(
      /import ['"]\.\/index\.css['"];?\s*import ['"]\.\/App\.css['"];?\s*import ['"]@discovery-box\/builder\/styles\.css['"];?/,
    );
    ...
    expect(cssImports.map(([, cssPath]) => cssPath)).toEqual([
      "./index.css",
      "./App.css",
      "@discovery-box/builder/styles.css",
    ]);
  });
